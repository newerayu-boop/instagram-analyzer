...
- `public/guides/` — **гайды для учеников курса** (светлый кремовый стиль, RU).
  Первый: `urok-1-gemini-notebooklm.html` + собранный из него `.pdf` (17 стр., A4).
  Вёрстка постранично (`.page` = A4), интерфейсы Gemini/NotebookLM нарисованы CSS-мокапами,
  кнопки помечены классом `.pin` (обводка + оранжевый кружок «нажми сюда») — маркер
  живёт внутри самого элемента, поэтому не съезжает при правках. Пересборка PDF:
  `/opt/pw-browsers/chromium --headless --no-pdf-header-footer --print-to-pdf=…` по
  `file://`-пути html (шрифты Inter берутся из `public/masterclass/fonts/`).
