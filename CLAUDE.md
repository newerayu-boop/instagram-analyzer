...
Спецэффекты: встреча при входе — **робот по присланному 3D-референсу**
(2026-08-10, картинка от Исламии): вязаный светло-серый капюшон надвинут,
под ним **большой глянцевый чёрный визор** во всю голову, сбоку круглый
наушник, из капюшона тонкая антенна, худи с завязками и карманом-кенгуру
(руки убраны в карман), тёмные джинсовые шорты с подвёрнутыми светлыми
манжетами, массивные кроссовки с розовым носком. Вязка — SVG-паттерн
`knitpat` поверх ткани; без него худи выглядит пластиком.
Порядок: **~1.25s точки-нейроны собирают силуэт сверху вниз** (задержка точки
зависит от её высоты + бегущая линия сканирования — не рой, а сборка) →
силуэт сменяется рисунком (SVG) → **глаза: сначала тёмные щели, потом
раскрываются** (`scaleY` от .08 с лёгким перелётом) **и горят розовым**
размытыми полосами (фильтр `eyeblur` + радиальный `bloom`) → проявляется
подпись `ASSALOMU ALAYKUM / ISLAMIYA / dizayner · AI-integrator · kurator`
под роботом → сайт. Клик пропускает. **Ни эмодзи, ни облачка реплики,
ни махания рукой — это специально убрано.** Точки нейронов берутся из тех же
фигур, по которым нарисован робот (`inEllipse`/`inQuad`/`inCapsule` в скрипте
заставки), поэтому силуэт совпадает с рисунком — при правке SVG правь и их.
Рамок-уголков нет.
Заставка видна из CSS сразу, а её скрипт стоит прямо за разметкой: если ждать
конца парсинга, на миг видно сам сайт.
Фото-роботы (`robot*.webp`, `robot.jpg`) больше не нужны, лежат как исходники.
Все 20 слайдов каруселей — бегущей 3D-волной (референс:
пин TIDY, ru.pinterest.com/pin/606086062398825953), в конце волны — макет
поста Instagram, где карусель листается сама; сайт мастер-класса — отдельная
секция «KATTA LOYIHA» во всю ширину (рамка браузера 1180px, автопрокрутка 30s,
пауза по ховеру), описание под ней.
**Никаких выдуманных метрик** (просмотры/лайки) на портфолио — только реальные
данные от Исламии. Секций «Обо мне»
