...
- `public/stratsessiya/` — **печатный справочник «Стратсессия» на узбекском (B1)**: 4 исходных
  ...
  свёрстан под A4, 33 страницы, доступен по `/stratsessiya`. Рядом — готовый `stratsessiya-uz.pdf`.
  ...
  **Графика:** все диаграммы — инлайн-SVG без библиотек (стековый бар тайминга разбора,
  гант 3-дневной сессии, метр «70%», отсортированный бар «100 часов», дорожная карта 0–7,
  пустая сетка для рисования выручки от руки). Палитра — одна зелёная ordinal-шкала
  `#4DC9A4 → #004334` (+ версия под тёмный фон), проверена валидатором скилла `dataviz`
  (`ALL CHECKS PASS`, light+dark). Одна шкала выбрана намеренно: читается при дальтонизме
  и не разваливается при ч/б печати. Правило вёрстки — **1 `.sheet` = ровно 1 лист A4**
  (держать высоту ≤ 255 мм, проверять рендером; при переполнении Chrome роняет футер
  на пустую страницу).
