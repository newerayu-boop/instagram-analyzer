...
Рамок-уголков нет.
**Движение робота** (2026-08-10, референс — пин ru.pinterest.com/pin/713257659759413787:
тёмный лендинг, 3D-робот на подиуме с переливающимся визором): робот стоит на
светящемся подиуме (`.pad`), медленно поворачивается (`botturn`, `rotateY` ±10°
с `perspective` на сцене) и по визору бежит радужный блик (`.sheen` —
полоса с градиентом `sheeng`, обтравка `visorclip` на внешней группе, размытие
`sheenblur`). Поворот включается **только после `built`**: до этого по рамке
`.bot` считаются координаты точек, и повёрнутый элемент дал бы неверный масштаб.
На фоне заставки — еле заметная техническая сетка.
**Творческое на самом сайте:** полоса прогресса чтения сверху (`.sprog`),
нумерация разделов `01 —` через CSS-счётчик в `.s-eyebrow` (номер и черта
выведены перед текстом через `order`, а не разметкой), бегущий перелив по
градиентным `em` в заголовках, наклон карточек работ с бликом под курсором
(только при `hover:hover` и без `prefers-reduced-motion`).
