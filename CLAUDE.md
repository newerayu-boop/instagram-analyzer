...
## `/qollanma` — гайд по Gemini и Gemini Notebook (2026-08-07)

`public/qollanma/index.html` — учебный гайд для участников курса, **на узбекском (B1)**,
светлая тема, один самодостаточный HTML (без внешних шрифтов/скриптов).
Два инструмента разведены по частям и в каждом блоке стоит **бейдж с логотипом**
(логотипы — inline-SVG symbols: Gemini-искра с градиентом, Notebook, Chrome, YouTube),
чтобы читатель не путал, о чём речь:
- **1-часть — Gemini** (что это, тарифы Free/Plus/Pro/Ultra + оплата из Узбекистана,
  модели Fast/Thinking, интерфейс, Deep Research, Gems, генерация изображений и права).
- **2-часть — Gemini Notebook** (интерфейс из трёх панелей, источники и лимиты,
  Студия: аудио/видео-обзор и язык вывода, заметки).
- **3-часть — Chrome-расширения** «YouTube to NotebookLM» (`kobncfkmjelbefaoohoblamnbackjggk`)
  и «NotebookLM Web Importer» (`ijdefdijdmghafocfmmdojfghnpelnfn`): установка и связка YouTube → Notebook.

**Важно по фактам:** с 16 июля 2026 NotebookLM переименован в **Gemini Notebook**
(тот же продукт, домен `notebooklm.google.com` работает) — в гайде это объяснено отдельно.
Цены/лимиты помечены как ориентировочные на август 2026.

Приём подачи: интерфейсы **нарисованы вёрсткой** (не скриншоты) — блок `.shot`
внутри `.shot-scroll` (горизонтальная прокрутка на мобильных), нумерованные пины `.pin`
со стрелкой-треугольником указывают на элемент, под макетом — список `.legend` с расшифровкой.
Такой же приём переиспользуй для любых будущих гайдов по интерфейсам.
