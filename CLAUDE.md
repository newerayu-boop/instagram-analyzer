...
Спецэффекты: встреча при входе — **робот в худи и наушниках, деловой**
(2026-08-10 переделан «посерьёзнее для бизнеса»: плечи шире головы, взрослые
пропорции, ноги и обувь, визор с тонкими полосами света вместо круглых глаз,
холодная графитовая гамма, никаких «мультяшных» приёмов). Порядок:
**~1.25s точки-нейроны собирают силуэт сверху вниз** (задержка точки зависит
от её высоты + бегущая линия сканирования — не рой, а сборка) → силуэт
сменяется рисунком (SVG) → **ровно загораются розовые глаза** (без дрожи) →
проявляется подпись `ASSALOMU ALAYKUM / ISLAMIYA / dizayner · AI-integrator ·
kurator` под роботом → сайт. Клик пропускает. **Ни эмодзи, ни облачка реплики,
