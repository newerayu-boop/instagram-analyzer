...
Спецэффекты: встреча при входе — **робот в худи и наушниках** (силуэт и подача
как у ai-sovga.vercel.app, стиль и цвет свои). Порядок: **~1s точки-нейроны
слетаются в силуэт робота** на canvas → силуэт сменяется рисунком (SVG) →
**загораются розовые глаза** (сначала тёмные гнёзда, потом вспышка с дрожью и
ровное свечение) → машет поднятой рукой и говорит «Assalomu alaykum!» → сайт.
Клик пропускает. Точки нейронов берутся из тех же фигур, по которым нарисован
робот (`inEllipse`/`inQuad`/`inCapsule` в скрипте заставки), поэтому силуэт
совпадает с рисунком — при правке SVG правь и их. Рамок-уголков нет.
