 ...
 *
 * Скрипт пишет в ту таблицу, внутри которой лежит, — id таблицы нигде не
 * задаётся. Поэтому подойдёт любая таблица в любом вашем аккаунте: создайте
 * пустую, вставьте этот код и запустите setup(). Заново переносить ничего не
 * нужно — все накопленные заявки перельются в неё сами.
 *
